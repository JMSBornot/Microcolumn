function out = izh_microcolumn_single_v2(N, M, xlsxFile, cfg)
%IZH_MICROCOLUMN_SINGLE  Single-microcolumn thalamocortical cortical model (MATLAB)
%
% out = izh_microcolumn_single_v2(N, M, xlsxFile, cfg)
%
...
%
% % Basic
% out = izh_microcolumn_single_v2(500, 80, 'neuron_data.xlsx');
% 
...
% out = izh_microcolumn_single_v2(800, 120, 'neuron_data.xlsx', cfg);
...
% Precompute comp chains per neuron for dendritic coupling
chains = cell(nNeur,1);
for n = 1:nNeur
    chains{n} = net.neuronCompStart(n) + (0:net.neuronCompCount(n)-1);
end
...
for it = 1:nSteps
    ...
    idxSTP = find(isFiniteTau);
    if ~isempty(idxSTP)
        syn.x(idxSTP) = syn.x(idxSTP) + single(dt .* ((1 - double(syn.x(idxSTP))) ./ syn.tauX(idxSTP)));
        syn.x(idxSTP) = min(max(syn.x(idxSTP), 0), 5);
    end
        ...
        if ~isempty(cfg.sensPulses)
            for r = 1:size(cfg.sensPulses,1)
                if tms >= cfg.sensPulses(r,1) && tms < cfg.sensPulses(r,2)
                    ...
                    if ~isempty(jS)
                        mask = (double(syn.srcCode(extIdx)) == jS);
                        hzNow(mask) = cfg.sensPulses(r,3);
                    end
                    if ~isempty(jT)
                        mask = (double(syn.srcCode(extIdx)) == jT);
                        hzNow(mask) = cfg.sensPulses(r,4);
                    end
                end
            end
        end
    ...
    for n = 1:nNeur
        ch = chains{n};
        if numel(ch) <= 1, continue; end
        Gup = net.neuronGup(n);
        Gdn = net.neuronGdown(n);

        % compartments are ordered by delay ascending (soma first)
        for q = 1:numel(ch)
            c = ch(q);
            I = 0;
            if q > 1
                mother = ch(q-1);
                I = I + Gdn * (v(c) - v(mother));
            end
            if q < numel(ch)
                daughter = ch(q+1);
                I = I + Gup * (v(c) - v(daughter));
            end
            Idendr(c) = I;
        end
    end
    ...
    % % NMDA voltage-dependence (Izhikevich supplementary material):
    % %   B(v) = z/(1+z),  z = ((v+80)/60)^2,  with v in mV.
    % z_nm = ((v + 80)./60).^2;
    % Bnmda = z_nm ./ (1 + z_nm);
